...
export default function More_place() {
  ...
  function handleClick(solutionNumber) {
    if (solutionNumber === 1) {
      ...
      setSolution(
        "<h3>Solution 3:</h3> <h4> Consider a '2' digit number in base 'n' representation.</h4> <p>Let it be (x y)n i.e., y + nx. Here, x and y range from 0 to n-1.</p> <p style='line-height:50px;'> Hence, the largest number that can be represented in 2 digits is when y=x=n-1. This number is (n-1) + (n-1)n = n2-1. Moreover, in the base n^2 representation, each digit ranges from 0 to n2-1. Thus, a base 'n^2' digit can be represented in base 'n' using 2 digits. So, an 'm' digit base 'n^2' number can be represented as a '2m' digit base 'n' number. Thus the answer to our problem is 2 x 1947 = 3894...</p><p>Since the base 'n^2' representation of the number referred by the question  has 1947 digits, it can be represented as :</p><p>(a1946  a1945   ...  a2  a1  a0) (base n^2 ) =  a0 +  a1 n^2  +   a2 n^4 +  ...  +  a1946 n3892</p><p>where the a is are base 'n^2' digits. Representing each ai as 2-digit base'n' numbers, (c<sub>i</sub>b<sub>i</sub>)n, we get a<sub>i</sub>=b<sub>i</sub>+c<sub>i</sub>n (b<sub>i</sub>,c<sub>i</sub> range from 0 to n-1). Now the number becomes</p><p>(a<sub>1946</sub>  a<sub>1945</sub>   ...  a<sub>2</sub>  a<sub>1</sub>  a<sub>0</sub>) (base n^2 ) =  (b<sub>0</sub>+c<sub>0</sub>n) +  (b<sub>1</sub>+c<sub>1</sub>n) x n2  +  ...  +  (b1946+c1946n) x n3892</p><p style='margin-left:200px;'>  =   b<sub>0</sub> + c<sub>0</sub>n + b<sub>1</sub>n^2 + ... + c<sub>1946</sub>n<sup>3893</sup> </p><p style='margin-left:200px;'> =>   there are 3894  base 'n' digits.</p>"
      );
    }
  }
